module Bimap_single_module(MapModule1 : Map.S)(MapModule2 : Map.S) = struct
  ...
  let union t ~f ~othermap =
    let newfwdmap = MapModule1.union f t.fwdmap othermap in
    let new_rev_map = create_reverse_map_from_forward_map newfwdmap in
    { fwdmap=newfwdmap; revmap=new_rev_map }
  let union_reverse t ~f ~othermap =
    let newrevmap = MapModule2.union f t.revmap othermap in
    let new_fwd_map = create_forward_map_from_reverse_map newrevmap in
    { fwdmap=new_fwd_map; revmap=newrevmap }
end;;
