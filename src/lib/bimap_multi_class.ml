...
module Bimap_multi_class (ModuleA : Map.S)(ModuleB : Map.S) = struct
  class bimap_multi_class = object(self)
    val mutable forward_map = ref ModuleA.Map.t;;
    val mutable reverse_map = ref ModuleB.Map.t;;
    ...
    method add ~key ~data =
      let update_maps () =
        if ModuleA.Map.mem !forward_map key then
          let oldvs = ModuleA.Map.find !forward_map ~key in
          let newvs = (data::oldvs) in 
          let () = forward_map :=
                     (ModuleA.Map.add !forward_map ~key ~data:newvs) in
          self#add_reverse ~key:data ~data:key
        else
          let () = forward_map :=
                     (ModuleA.Map.add !forward_map ~key ~data) in
          self#add_reverse ~key:data ~data:key in 
      if ModuleA.Map.mem !forward_map key then
        if List.mem
             (ModuleA.Map.find !forward_map key) data then ()
        else 
          update_maps ()
      else
        update_maps ()
    ...
    method add_reverse ~key ~data =
      let update_maps () =
        let () = reverse_map := (ModuleB.Map.add !reverse_map ~key ~data) in
